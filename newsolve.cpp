...
void solve()
{
    ...
    FOR(i, 0, n)
    {
        ...
        {
            if (mp[vt[i]][0] < i - mp[vt[i]][2])
            {
                mp[vt[i]][1] = mp[vt[i]][0];
                mp[vt[i]][0] = i - mp[vt[i]][2];
            }
            else if (mp[vt[i]][1] < i - mp[vt[i]][2])
                mp[vt[i]][1] = i - mp[vt[i]][2];
            mp[vt[i]][2] = i;
        }
        else
        {
            mp[vt[i]][2] = i;
            mp[vt[i]][0] = mp[vt[i]][1] = -1;
        }
    }
    ll ans = 1e6;
    trav(i, mp)
    {
        ...
        if (arr[0] = -1)
        {
            // temp = max(max(arr[2], n - arr[2]) / 2, min(arr[2], n - arr[2]));
            temp = (n - arr[2]) / 2;
        }
        else
        {
            temp = max(arr[0] / 2, arr[1]);
        }
        ans = min(temp, ans);
    }
    cout << ans << endl;
