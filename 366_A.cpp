...
void solve()
{
    ...
    FOR(i, 0, 4)
    {
        ...
        if (ans > min(arr[i][0], arr[i][1]) + min(arr[i][3], arr[i][2]))
        {
            ans = min(arr[i][0], arr[i][1]) + min(arr[i][3], arr[i][2]);
            ind = i + 1;
        }
    }
    if (ans > n)
        ...
        cout << ind << " " << min(arr[ind - 1][0], arr[ind - 1][1]) << " " << n - min(arr[ind - 1][0], arr[ind - 1][1]) << endl;
}
signed main()
{
