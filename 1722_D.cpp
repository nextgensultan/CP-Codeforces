...
{
    int i = 1;
    while (i < sz(vt) && vt[i] == vt[i - 1])
        i++;
    if (vt[i] <= vt[i - 1])
    {
        int j = i;
        while (j < sz(vt) && vt[j] <= vt[j - 1])
            j++;
        return j;
    }
    if (vt[i] >= vt[i - 1])
...
void solve()
{
    ...
    int ind = GetAns(vt);
    if (ind == n)
        cout << (vt[0] <= vt[n - 1] ? 0 : vt[0] + 1) << endl;

    else
    {
        int diff = (vt[ind] + vt[ind - 1] - 1) / 2 + 1;
        bool flag = true;
        FOR(i, 1, n)
        {
            flag &= (abs(vt[i] - diff) >= abs(vt[i - 1] - diff));
        }
        cout << (flag ? diff : -1) << endl;
    }
}
signed main()
{
