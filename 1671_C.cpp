...
void solve()
{
    ...
    //     if (ans > x)
    //         break;
    //     int ind = lower_bound(all(vt), x, [&](long long &elem, const ll &v)
    //                           { ll temp = distance(&vt[0],&elem) +1;
    //                             ll comp = elem + i * temp;
    //                             return  comp <= v; }) - vt.begin();
    //     sum += ind;
    // }
    // cout << endl
    //      << sum << endl;
    ...
    FOR(i, 0, n)
    {
        ...
        ll ans = max((ll)0, (x - pre[i] + i + 1)) / (i + 1);
        sum += ans;
    }
    cout << sum << endl;
}
